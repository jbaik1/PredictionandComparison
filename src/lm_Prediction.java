...
public class lm_Prediction {
	...
	{
		lm_Prediction pred = new lm_Prediction();
		...
		System.out.println("Running lm_Prediction\n");
	...
	public void checkTier(File f, PrintWriter writer)
	{
		...
		String line = "";
		String phoneme = "";
		ArrayList<String> LM_Labels = new ArrayList<String>();
		...
		while (fin.hasNextLine())
		{
			...
			else if (line.indexOf("LEXI phoneme") != -1)
			{
				...
				LM_Labels = convertLEXI(lexi);
				for (String label: LM_Labels)
				{
	...
	public ArrayList<String> convertLEXI(ArrayList<String> phones)
	{
		...
		{
			matchLabel(phones.get(i) ,lm, i);
		}
		
		return lm;
	}
	
	public int matchLabel(String phone, ArrayList<String> lm, int index)
	{

		for (String test: new String[] {"ao", "aa", "iy", "uw", "eh", "ih", "uh", "ah",
				"ax", "ae", "ey", "ow", "er"})
		{
			if (phone.equals(test))
			{
				lm.add("V");
				labelTimes.add((phoneStartTimes.get(index) + phoneEndTimes.get(index))/2);
				return 0;
			}
		}
		for (String test: new String[] {"ay", "aw", "oy"})
		{
			if (phone.equals(test))
			{
				lm.add("V");
				labelTimes.add((phoneStartTimes.get(index) + phoneEndTimes.get(index))/2);
		//		lm.add("G");
		//		labelTimes.add(phoneEndTimes.get(index) - 0.001);
				return 0;
			}
		}
		for  (String test: new String[] {"w", "y","r", "l", "h"})
		{
			if (phone.equals(test))
			{
				lm.add("G");
		...
		for  (String test: new String[] {"f", "th", "s", "sh", "v", "dh", "z", "zh"})
		{
			if (phone.equals(test))
			{
				lm.add("Fc");
		...
		for  (String test: new String[] {"p", "t", "k", "b", "d", "g"})
		{
			if (phone.equals(test))
			{
				lm.add("Sc");
		...
		for  (String test: new String[] {"ch", "jh"})
		{
			//Sc at start time; Sr at midpoint, Fc
			//at midpoint+1msec; Fr at end point
			//affricates; Sr and Fc occur
			...
			if (phone.equals(test))
			{
				...
				labelTimes.add((phoneStartTimes.get(index) + phoneEndTimes.get(index))/2);
				lm.add("Fc");
				labelTimes.add((phoneStartTimes.get(index) + phoneEndTimes.get(index))/2 + 0.001);
				lm.add("Fr");
				labelTimes.add(phoneEndTimes.get(index));
				return 0;
			}
